...
async function sendMsg(data, send, msg) {
  ...
  for (let i of msg) {
    ...
    switch (i.type) {
      ...
      case "image":
        ...
        ret = await send(2, await uploadFile(data, i.data.file), quote, at)
        break
      case "record":
        logger.info(`${logger.blue(`[${data.self_id}]`)} 发送音频：${i.data.file.replace(/^base64:\/\/.*/, "base64://...")}`)
        ret = await send(8, await uploadFile(data, i.data.file), quote, at)
        break
      case "video":
        logger.info(`${logger.blue(`[${data.self_id}]`)} 发送视频：${i.data.file.replace(/^base64:\/\/.*/, "base64://...")}`)
        ret = await send(3, await uploadFile(data, i.data.file), quote, at)
        break
      case "reply":
        quote = i.data.id
        break
...
function sendFriendMsg(data, msg) {
  ...
  return sendMsg(data, (type, content, quote) => data.bot.API.directMessage.create(type, data.user_id, content, quote), msg)
}

function sendGroupMsg(data, msg) {
  logger.info(`${logger.blue(`[${data.self_id}]`)} 发送群消息：[${data.group_id}]`)
  return sendMsg(data, (type, content, quote, at) => data.bot.API.message.create(type, data.group_id, content, quote, at), msg)
}

async function recallMsg(data, recall, message_id) {
...
async function connectBot(token) {
  ...
  Bot[id].pickFriend = user_id => {
    ...
    return {
      ...
      recallMsg: message_id => recallMsg(i, message_id => i.bot.API.directMessage.delete(message_id), message_id),
  ...
  Bot[id].pickGroup = group_id => {
    ...
    return {
      ...
      recallMsg: message_id => recallMsg(i, message_id => i.bot.API.message.delete(message_id), message_id),
